...
def create_main_window():
    ...
    config.remove_button = tk.Button(root, text=config.texts[config.current_lang]["remove_file"], command=lambda: remove_selected(file_list, config.file_listbox, part1_entry))
    config.remove_button.place(x=316, y=420)
    ...
    config.select_file_button = tk.Button(root, text="Vyber soubory", command=lambda: select_files(file_list, part1_entry))
    config.select_file_button.place(x=20, y=420)

    # Creation of label on file name text field
    config.part1_label = tk.Label(root, text=config.texts[config.current_lang]["name_label"])
    config.part1_label.place(x=20, y=10)
    ...
    config.move_up_button = tk.Button(root, text="△", command=lambda: move_up(file_list, config.file_listbox))
    config.move_up_button.place(x=525, y=60, width=30, height=30)

    # Creation of button to move one down
    config.move_down_button = tk.Button(root, text="▽", command=lambda: move_down(file_list, config.file_listbox))
    config.move_down_button.place(x=525, y=385, width=30, height=30)

    # Creation of button to move top
    config.move_to_top_button = tk.Button(root, text="Top", command=move_to_top)
    config.move_to_top_button.place(x=525, y=90, width=50, height=30)

    # Creation of button to move bottom
    config.move_to_bottom_button = tk.Button(root, text="Bottom", command=move_to_bottom)
    config.move_to_bottom_button.place(x=525, y=355, width=50, height=30)

    # Creation of label on rename type
    config.part2_label = tk.Label(root, text="Metoda:")
    config.part2_label.place(x=300, y=10)
    ...
    config.rename_button = tk.Button(root, text="Přejmenuj soubory", command=lambda: rename_files(part1_entry.get(), counter_type.get(), file_list))
    config.rename_button.place(x=115, y=420)
